...
// PLACE BID
// =========================
// backend/routes/auctionRoutes.js - Update the bid endpoint
      ...
      if (previousBidder.bidderId.toString() !== bidderId) {
        try {
          const notification = new Notification({
            ...
            data: { auctionId: auction._id, newBid: amount, fishType: auction.fishType },
            actionUrl: `/auction`,
            priority: 'high'
          });
...
// =========================
// UPDATE AUCTION (Admin) - Edit quantity, price, location
// =========================
router.put("/:id/update", async (req, res) => {
  try {
    const { id } = req.params;
    const { quantity, startingPrice, currentBid, location, description, fishType } = req.body;
    ...
    // Update quantity if provided and auction is not live
    if (quantity !== undefined && quantity > 0) {
      // For live auctions, we cannot change quantity
      if (auction.status === "Live") {
        return res.status(400).json({
          success: false,
          message: "Cannot change quantity for live auction"
        });
      }
      updateData.quantity = Number(quantity);
    }
    
    // Update starting price if provided
    if (startingPrice !== undefined && startingPrice > 0) {
      updateData.startingPrice = Number(startingPrice);
      // If auction is not live yet, also update current bid to match starting price
      if (auction.status !== "Live") {
        updateData.currentBid = Number(startingPrice);
      }
    }
    ...
    // Update fish type if provided
    if (fishType !== undefined && fishType.trim() !== "") {
      updateData.fishType = fishType.trim();
    }
    ...
    // Create notification for seller about auction update
    if (auction.status !== "Live") {
      await Notification.create({
        userId: auction.sellerId,
        type: "auction_updated",
        title: "📝 Auction Updated",
        message: `Your auction for ${auction.fishType} has been updated by admin.`,
        relatedId: auction._id,
        relatedModel: "Auction"
      });
    }

    console.log(`✅ Auction updated: ${updatedAuction._id}`);
